...
- **The API Gateway path has a hard 30-second ceiling that Lambda's own timeout can't override.** `mercato-agent`'s Lambda timeout is set to 28s (`infra/deploy_lambda.py`), because the HTTP API integration in front of it caps out at 30s — and per [AWS's own quota table](https://docs.aws.amazon.com/apigateway/latest/developerguide/http-api-quotas.html), that maximum **cannot be increased**, for any HTTP API, by any means. `infra/setup_api_gateway.py` doesn't set `TimeoutInMillis` explicitly because there's nothing to set it *to* — 30000ms is already the ceiling AWS enforces. A turn that takes longer than 30 seconds either gets cut off by the Lambda's own 28s timeout or, previously, ran past it, got billed, and was thrown away anyway: the caller gets a 504 regardless of whether the model was still working or already had an answer. Measured production durations have hit 44.6s and 53.3s, both past this line. There is no config fix on the API Gateway side — the only lever is keeping turns under 30s in the first place, which is why the [UCP fix](#-live-product-discovery) (removing a mandatory round-trip to a dead endpoint from every discovery turn) is a latency fix and not just a cost one. This constraint doesn't apply to the CLI, which runs the agent in-process and isn't bounded by this Lambda or this API at all.
